################################################################################
def prepare_squared_distance_file(data_file,variable_name,N,D,measure_type='dSq',
...
################################################################################
  ...
  t0 = time.time()
  job_id = '<Squared Distance File Preparation>'
  ...
  from sparsify import multi_smallest_items_in_each_row_of_table
  ...
  if (my_rank==0):
    yRow = [np.array([[]],dtype=int)]*len(nN)
    yCol = [np.array([[]],dtype=int)]*len(nN)
    yVal = [np.array([[]])]*len(nN)
  
  # yRow,yCol 1-based
  for row in my_row:
    yVal_mine = collect_concat_measure(n,row=row)
    yRow_mine,yCol_mine,yVal_mine = \
      multi_smallest_items_in_each_row_of_table(yVal_mine,nN)
    for i, nN_i in enumerate(nN):
      yRow_mine[i] += row*n
      if (my_rank==0):
        yRow[i] = np.hstack((yRow[i],yRow_mine[i]))
        yCol[i] = np.hstack((yCol[i],yCol_mine[i]))
        yVal[i] = np.hstack((yVal[i],yVal_mine[i]))
        for worker in range(1,group_size):
          ...
          yRow[i] = np.hstack((yRow[i],yRow_worker[i]))
          yCol[i] = np.hstack((yCol[i],yCol_worker[i]))
          yVal[i] = np.hstack((yVal[i],yVal_worker[i]))
      else:
  ...
  if (my_rank==0):
    ...
    for i, nN_i in enumerate(nN):
      yRow[i],yCol[i],yVal[i] = symmetrize(yRow[i],yCol[i],yVal[i])
      sqDist_file_name = sqDist_file + '_nN_' + str(nN_i) + '.h5'
      write_h5(sqDist_file_name,yRow[i],'yRow')
      write_h5(sqDist_file_name,yCol[i],'yCol')
      write_h5(sqDist_file_name,yVal[i],'yVal')
...
if __name__=="__main__":
  ...
  prepare_squared_distance_file(data_file,variable_name,N,D,measure_type,c,h5,transpose,n,nN,sqDist_file,cleanup,no_block,run_mpi)
